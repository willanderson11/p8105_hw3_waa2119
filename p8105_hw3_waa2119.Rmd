...
The number of missing data in the prcp column is `r sum(is.na(ny_noaa$prcp))`

The number of missing data in the prcp column is `r sum(is.na(ny_noaa$snwd))` 

The number of missing data in the prcp column is `r sum(is.na(ny_noaa$tmax))` 

The number of missing data in the prcp column is `r sum(is.na(ny_noaa$tmin))`
...
The most common snowfall values are `r most_snow`. The most common snowfall value is 0 mm because New York has many days in which it does not receive snow across the state. The next most common snowfall estimates are relatively small numbers as well since New York does not usually receive large amounts of snow each year. 
...
The average temperatures are observably higher in July in New York compared to January, some years contain outliers as January can be seen having anomalous low temperatures and July has years with anomalous high temperatures outside the main cluster of data points. 
